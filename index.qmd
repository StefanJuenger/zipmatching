...
centroid_map <- (map1 | map2) + patchwork::plot_layout(guides = "collect", ncol = 2) +
  patchwork::plot_annotation(title = "Accuracy of Centroid Linking")

# Save the combined plot
ggplot2::ggsave("./test/centroid_map.png", centroid_map, dpi = 300, width = 8, height = 6, units = "in")
...
color_mapping2 <- c(
  ...
  "Overlap" = viridis::viridis(5)[4]          # Color for Overlap
)
...
ggplot2::ggplot() +
  ...
  ggpattern::geom_sf_pattern(data = overlap, aes(fill = factor("Overlap", levels = desired_order2)), 
...
For advanced users, we would also like to point out the package [`areal`](https://github.com/chris-prener/areal) that builds upon the `sf`package we use here and expands the functions for area-weighted interpolations [@prener_areal_2020].
...
As shown in the plot, there are no substantial or significant differences in the correlation coefficients between the various matching methods or when compared to the ZIP code-level population density. This suggests that the linking processes perform similarly well in this case.
...
custom_labels <- c(
  ...
  "interpolation_inhabitants_mun" = "Areal interpolation"
...
In this tool application, we evaluated three matching methods—centroid matching, areal matching, and area-weighted interpolation—to link survey data with geospatial context variables in order to investigate whether population density is associated with support for far-right parties in Germany.
