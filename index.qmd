...
ggplot() +
  geom_sf(data = municipality1, aes(fill = factor("Municipality 1", levels = desired_order)), alpha = 0.4, color = "black") +
  geom_sf(data = fake_zip_codes, aes(fill = factor("Zip Code", levels = desired_order)), alpha = 0.8, color = "black") +
 
  scale_fill_manual(values = color_mapping, name = "Layers") +
...
map1 <- ggplot() +
  geom_sf(data = municipality1, aes(fill = factor("Municipality 1", levels = desired_order1)), alpha = 0.4, color = "black") +
  geom_sf(data = fake_zip_codes, aes(fill = factor("Zip Code", levels = desired_order1)), alpha = 0.8, color = "black") +
  geom_sf(data = fake_zip_codes_centroid, aes(fill = factor("Centroid", levels = desired_order1)), shape = 20, color = "black", size = 4) +
...
map2 <- ggplot() +
  geom_sf(data = municipality2, aes(fill = factor("Municipality 1", levels = desired_order1)), alpha = 0.4, color = "black") +
  geom_sf(data = fake_zip_codes, aes(fill = factor("Zip Code", levels = desired_order1)), alpha = 0.8, color = "black") +
  geom_sf(data = fake_zip_codes_centroid, aes(fill = factor("Centroid", levels = desired_order1)), shape = 20, color = "black", size = 4) +
  scale_fill_manual(values = color_mapping1, name = "Layers") +
...
create_overlap_polygon <- function() {
  coords <- matrix(c(
    1.02, 0.02,     # Bottom-left
    2.98, 0.52,    # Bottom-right
    2.98, 1.48,     # Top-right
    1.02, 1.48,     # Top-left
    1.02, 0.02       # Closing the polygon
  ), ncol = 2, byrow = TRUE)
...
# Create a named vector for fill colors
color_mapping2 <- c(
  "Municipality 1" = viridis_colors[1],  # Color for Municipality 1
  "Municipality 2" = viridis_colors[2],  # Color for Municipality 2
  "Zip Code" = viridis_colors[3],        # Color for Zip Code
  "Overlap" = viridis_colors[4]          # Color for Overlap
)
...
# Create a plot
ggplot() +
  geom_sf(data = municipality1, aes(fill = factor("Municipality 1", levels = desired_order2)), alpha = 0.3, color = "black") +  # Color for Municipality 1
  geom_sf(data = municipality2, aes(fill = factor("Municipality 2", levels = desired_order2)), alpha = 0.3, color = "black") +  # Color for Municipality 2
  geom_sf(data = zip_code1, aes(fill = factor("Zip Code", levels = desired_order2)), alpha = 0.8, color = "black") +      # Color for Zip Code
  geom_sf(data = overlap, aes(fill = factor("Overlap", levels = desired_order2)), alpha = 0.8, color = "black", size = 1.5) +  # Color for Overlap
...
color_mapping3 <- c(
  ...
  "Municipality 2" = viridis_colors[2],     # Color for Municipality 2
  "Zip Code" = viridis_colors[3],           # Color for Zip Code
  "Shares Municipality 1" = viridis_colors[4],  # Color for Shares of Municipality 1
  "Shares Municipality 2" = viridis_colors[5]   # Color for Shares of Municipality 2
)
...
ggplot() +
  geom_sf(data = municipality1, aes(fill = factor("Municipality 1", levels = desired_order3)), alpha = 0.3, color = "black") +  # Color for Municipality 1
  geom_sf(data = municipality2, aes(fill = factor("Municipality 2", levels = desired_order3)), alpha = 0.3, color = "black") +  # Color for Municipality 2
  geom_sf(data = zip_code, aes(fill = factor("Zip Code", levels = desired_order3)), alpha = 0.7, color = "black") +      # Color for Zip Code
  geom_sf(data = interpolated_municipality1, aes(fill = factor("Shares Municipality 1", levels = desired_order3)), alpha = 0.9, color = "black") +  # Shares for Municipality 1
  geom_sf(data = interpolated_municipality2, aes(fill = factor("Shares Municipality 2", levels = desired_order3)), alpha = 0.9, color = "black") +  # Shares for Municipality 2
...
It is important to note that we focus on the differences in the number of inhabitants <!-- population density --> per municipality to evaluate the matching process because that is our main variable of interest.
...
First, we plot the differences between our variable of interest, number of inhabitants, for each matching method (centroid matching, area matching, and area-weighted interpolation) compared with the real estimate based on the point locations. 
...
To do this, we plot the densities of the scaled differences across the different matching methods.
...
Wile the location of the peak tells us about the magnitude of the differences, the width of the distribution reflects the variability of those differences.

When comparing centroid and areal matching, we observe a slightly wider density plot, suggesting that these methods may perform differently in certain areas. 
In contrast, the other plots indicate small and consistent differences, implying that the respective methods perform similarly across the data.
...
```
To further assess the performance of the matching methods in different areas, we create a map that allows us to identify which method is most consistently successful at minimizing population differences across ZIP codes. The map shows which method results in the smallest difference when compared to the actual population values, as estimated via the simulated points.

We observe that different matching methods perform better in certain regions. For example, in densely populated areas like cities, ZIP codes are often contained within a single municipality, so centroid matching performs particularly well in these areas. On average, areal interpolation, which takes multiple factors into account, performs the best across the entire study area.
...
<!-- hier kann man leider wenig sehen: ggfs. doch mit gesamt karte austauschen? -->
...
<!-- brauchen population density! -->

Finally, we aim to assess whether and how the different matching methods influence the answers to our research question. Rather than developing an elaborate model, we take a more straightforward approach by examining the correlation between our variable of interest—the AfD rating — and the matched context variable, population density.
In this example, we can compare the correlation coefficients across the different matching methods and with population density at the zip code level, as we have access to this information.

As shown in the plot, there are no substantial or significant differences in the correlation coefficients between the various matching methods or when compared to the zip code-level population density. This suggests that the linking process perform similarly well in this case.

From a substantive perspective, the correlation is consistently small across all comparisons. Specifically, we observe that living in areas with higher population density is weakly correlated with lower AfD ratings. 
In other words, individuals living in more densely populated areas tend to support the AfD less—though the strength of this relationship is modest.
...
# Calculate pairwise correlations with 'afd_rating' and columns containing 'inhabitants'
correlations <- linked_survey |>
  dplyr::select(afd_rating, contains("inhabitants")) |>
...
correlations_long <- correlations %>%
  ...
  # Keep only correlations with 'afd_rating'
  filter(x == "afd_rating" & x != y)

# Create a custom label mapping (this is just an example)
custom_labels <- c(
  "inhabitants_zip_code" = "ZIP code",
  "cent_inhabitants_mun" = "Centroid matching",
  "areal_inhabitants_mun" = "Areal matching",
  "interpolation_inhabitants_mun" = "Areal interploation"
  # Add more mappings as needed
)

# Plot the correlation coefficients as a bar chart with correlation values as labels
ggplot(correlations_long, aes(x = y, y = value, fill = value)) +
  geom_bar(stat = "identity", show.legend = FALSE) +
  scale_fill_viridis(option = "D") +  # Apply viridis color scale
  labs(title = "Correlation of Population Density Variables with AFD Rating",
...
In this tool application, we evaluated three matching methods—centroid matching, area matching, and area-weighted interpolation—to link survey data with geospatial context variables in order to investigate whether population density is associated with support for far-right parties in Germany.

Our findings indicate that, while all three methods show slight deviations from the real population estimates (derived from simulated point locations), they generally produce similar results in terms of accuracy. In terms of consistency, subtle differences were observed: centroid matching performed slightly better in densely populated areas, while area-weighted interpolation yielded the best results on average across the entire study area. However, despite these variations, the choice of matching method had minimal impact on the research outcomes, including the correlation between AfD ratings and population density.

Our top three recommendations (tbd mit Stefan):

- One
- Two
- Three

Choosing a matching method, however, is only one aspect of the researcher's decision-making process and does not absolve them from the conceptual considerations inherent in the linking process: how to define the "neighborhood".

When defining neighborhoods, researchers must decide which geographic unit best captures the relevant social or environmental context. Different definitions of "neighborhood" can lead to different conclusions about the relationships between spatial context and outcomes like political preferences. 
For example, the uncertain geographic unit problem (UGP) highlights the importance of carefully considering spatial boundaries in social research. The choice of neighborhood unit can significantly affect the results, particularly when studying phenomena such as political participation or support for far-right parties.

The methods evaluated in this application provide different ways of addressing the challenges posed by mismatched or incongruent boundaries when defining a "neighborhood." However, researchers must remain aware that the linking process should be guided by a clear understanding of how these spatial definitions shape the conclusions drawn from integrated survey and contextual data.
