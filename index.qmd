...
The linking process is crucial to answer this research question but is prone to certain pitfalls, such as incongruent geospatial units, e.g. overlapping geospatial units that do not share a common border(Qiu, Zhang, and Zhou 2012)https://bookdown.org/robinlovelace/geocompr/spatial-operations.html#ref-qiu_development_2012.
...
- The geometries of the areal units that include the context information they are interested in
...
map2 <- ggplot() +
  geom_sf(data = municipality2, aes(fill = factor("Municipality", levels = desired_order)), alpha = 0.75, color = "black") +
  geom_sf(data = fake_zip_codes, aes(fill = factor("Fake Zip Codes", levels = desired_order)), alpha = 0.5, color = "black") +
  geom_sf(data = fake_zip_codes_centroid, aes(fill = factor("Centroid", levels = desired_order)), shape = 20, color = "red", size = 3) +
  scale_fill_manual(values = color_mapping1, name = "Layers") +
...
## Aggregate at the Zip Code level
